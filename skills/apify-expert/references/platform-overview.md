...
- **Billing unit:** compute = memory-GB × runtime-hours (plus proxy, storage operations, data transfer). PPE Actors add `Actor.charge()` events on top (see the Apify monetization docs).
...
- REST API, tokens, MCP access pattern → `references/api-and-integrations.md`
- Standby mode, Store mechanics, account limits → `references/platform-features.md`
