...
- **`isSourceCodeHidden`** hides source in the Console UI **but the API still exposes `versions[0].sourceFiles[]`** to authenticated users — keep any non-shippable files out of the push entirely.
...
2. **Per-Actor MCP (Standby)** — an MCP server Actor exposes its own tools at `https://<user>--<actor-name>.apify.actor/mcp` while running in Standby.
...
- Standby/MCP platform behavior → `references/platform-features.md`
- Memory↔CPU, storage, proxy → `references/platform-overview.md`
